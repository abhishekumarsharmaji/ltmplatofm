...
**How to apply:** Generate join tokens only on the server after ownership or enrollment checks. Grant publishing only to the owning creator/admin host and keep students subscribe-only. Do not hide or block an active class merely because its scheduled end time passed; the manual end action must mark it completed and close the LiveKit room. Treat transient LiveKit disconnects as recoverable: request a fresh token, retry in a bounded way, expose a manual reconnect, and keep safe disconnect diagnostics visible. Automatic recording must wait for confirmed `EGRESS_ACTIVE`, store privately in R2, and become a lesson only after Egress completes and the object exists. This environment has returned Egress error 412, “Start signal not received”; that indicates LiveKit Egress server connectivity/configuration rather than an app success. Keep direct secure lesson-video upload available and never fake recording readiness.