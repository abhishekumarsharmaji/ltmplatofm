...
const pageData = (title: string, description: string, canonical: string, graph: unknown, body: string) =>
  ({ title, description, canonical, graph, body });
const render = async (res: any, data: ReturnType<typeof pageData>) => {
  ...
  const head = `<title>${esc(data.title)}</title><meta name="description" content="${esc(data.description)}"><meta name="robots" content="index,follow"><link rel="canonical" href="${esc(data.canonical)}"><meta property="og:title" content="${esc(data.title)}"><meta property="og:description" content="${esc(data.description)}"><meta property="og:type" content="website"><meta property="og:url" content="${esc(data.canonical)}"><meta property="og:site_name" content="CoreSkils"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${esc(data.title)}"><meta name="twitter:description" content="${esc(data.description)}"><script type="application/ld+json">${json(data.graph)}</script>`;
...
router.get("/sitemap.xml", async (_req, res) => {
  const [courses, products, creators] = await Promise.all([
    db.select({ slug: coursesTable.slug, updatedAt: coursesTable.updatedAt }).from(coursesTable).where(eq(coursesTable.status, "published")),
...
router.get(["/", "/courses", "/products"], async (req, res) => {
  ...
  const [courses, products] = await Promise.all([
    db.select({ title: coursesTable.title, description: coursesTable.description, slug: coursesTable.slug }).from(coursesTable).where(eq(coursesTable.status, "published")).orderBy(desc(coursesTable.updatedAt)).limit(50),
...
router.get("/courses/:idOrSlug", async (req, res) => {
  ...
  const [row] = await db.select({ course: coursesTable, product: productsTable, profile: creatorProfilesTable, creatorName: usersTable.name }).from(coursesTable).innerJoin(usersTable, eq(usersTable.id, coursesTable.creatorId)).leftJoin(creatorProfilesTable, eq(creatorProfilesTable.userId, coursesTable.creatorId)).leftJoin(productsTable, and(eq(productsTable.courseId, coursesTable.id), eq(productsTable.type, "course"), eq(productsTable.status, "published"))).where(and(eq(coursesTable.status, "published"), /^\d+$/.test(key) ? eq(coursesTable.id, Number(key)) : eq(coursesTable.slug, key)));
  if (!row) { res.status(404).send("Course not found"); return; }
  const c = row.course, description = text(c.description, 300), path = `/courses/${c.slug}`, author = row.profile ? stripProfile(row.profile, row.creatorName) : { name: row.creatorName };
  const questions = faq(c.faqs), graph: any = { "@context": "https://schema.org", "@type": "Course", name: c.title, description, url: url(path), provider: { "@type": "Person", name: row.creatorName }, offers: row.product ? { "@type": "Offer", price: row.product.priceMinor / 100, priceCurrency: row.product.currency, availability: "https://schema.org/InStock", url: url(path) } : undefined, breadcrumb: { "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Courses", item: url("/courses") }, { "@type": "ListItem", position: 2, name: c.title, item: url(path) }] } };
  if (questions.length) graph.hasPart = { "@type": "FAQPage", mainEntity: questions.map((x) => ({ "@type": "Question", name: x.question, acceptedAnswer: { "@type": "Answer", text: x.answer } })) };
  await render(res, pageData(`${c.title} | CoreSkils`, description, url(path), graph, `<main><nav><a href="/courses">Courses</a></nav><article><h1>${esc(c.title)}</h1><p>${esc(description)}</p><p>Created by <a href="/creators/${esc(row.profile?.username || "")}">${esc(row.creatorName)}</a></p></article></main>`));
});
...
router.get("/products/:idOrSlug", async (req, res) => {
  ...
  const graph: any = { "@context": "https://schema.org", "@type": "Product", name: p.title, description, url: url(path), brand: { "@type": "Brand", name: "CoreSkils" }, offers: { "@type": "Offer", price: p.priceMinor / 100, priceCurrency: p.currency, availability: "https://schema.org/InStock", url: url(path) }, breadcrumb: { "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Products", item: url("/products") }, { "@type": "ListItem", position: 2, name: p.title, item: url(path) }] } };
  if (questions.length) graph.mainEntityOfPage = { "@type": "FAQPage", mainEntity: questions.map((x) => ({ "@type": "Question", name: x.question, acceptedAnswer: { "@type": "Answer", text: x.answer } })) };
  await render(res, pageData(`${p.title} | CoreSkils`, description, url(path), graph, `<main><nav><a href="/products">Products</a></nav><article><h1>${esc(p.title)}</h1><p>${esc(description)}</p><p>By ${esc(row.creatorName)}</p></article></main>`));
});
...
router.get("/creators/:username", async (req, res) => {
  ...
  const courses = await db.select({ title: coursesTable.title, slug: coursesTable.slug, description: coursesTable.description }).from(coursesTable).where(and(eq(coursesTable.creatorId, row.profile.userId), eq(coursesTable.status, "published")));
