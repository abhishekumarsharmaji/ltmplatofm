    ...
    if (isOwned) {
      return (
        <Button 
          ...
          className="w-full h-[56px] bg-primary hover:bg-[#10A364] text-white font-medium rounded-lg text-[16px] shadow-[0_8px_24px_rgba(21,207,116,0.25)] transition-transform hover:-translate-y-0.5"
        >
          Go to My Library
        </Button>
      );
  ...
  return (
    <PublicLayout>
      <div className="bg-[#FAFAFA] min-h-screen text-black">
        {/* Hero Section */}
        <section className="bg-[#002333] pt-32 pb-24 lg:pt-40 lg:pb-32 text-white relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 md:px-8 relative z-10">
            <div className="grid lg:grid-cols-12 gap-12 lg:gap-20 items-center">
              ...
              <div className="lg:col-span-7 space-y-8">
                ...
                <h1 className="text-[40px] lg:text-[56px] font-bold tracking-tight leading-[1.1]">
                  {product.title}
                </h1>
                
                {salesPage?.tagline && (
                  <p className="text-[20px] lg:text-[24px] text-[#A0B0C0] font-medium leading-relaxed max-w-2xl">
                    {salesPage.tagline}
                  </p>
                )}
                ...
                {salesPage?.benefits && salesPage.benefits.length > 0 && (
                  <ul className="space-y-3 pt-4">
                    {salesPage.benefits.map((benefit, i) => (
                      <li key={i} className="flex items-start gap-3 text-[16px] text-[#E4E4E4]">
                        <CheckCircle2 className="w-6 h-6 text-[#10A364] shrink-0" />
          ...
          <div className="container mx-auto px-4 md:px-8">
            <div className="grid lg:grid-cols-12 gap-12 lg:gap-20">
              ...
              <div className="lg:col-span-7 space-y-20">
                {/* Description */}
                {product.description && (
                  <div className="prose prose-lg max-w-none text-[#394649]">
                    <p className="text-[18px] leading-relaxed whitespace-pre-line">{product.description}</p>
                  </div>
                )}
                  ...
                  <div className="space-y-16">
                    {salesPage.sections.map((section, index) => (
                      <div key={index} className="space-y-6">
                        <h2 className="text-[28px] lg:text-[32px] font-bold text-black leading-tight">{section.heading}</h2>
                        <div className="prose prose-lg max-w-none text-[#394649]">
                          <p className="whitespace-pre-line leading-relaxed text-[16px]">{section.body}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {salesPage?.testimonials && salesPage.testimonials.length > 0 && (
                  <div className="space-y-8 pt-8 border-t border-[#E5E5E5]">
                    <h2 className="text-[28px] font-bold text-black flex items-center gap-3">
                      <MessageSquareQuote className="w-8 h-8 text-primary" />
                      What people are saying
                    </h2>
                    <div className="grid sm:grid-cols-2 gap-6">
                      {salesPage.testimonials.map((t, index) => (
                        <div key={index} className="bg-white p-6 rounded-xl border border-[#E5E5E5] shadow-sm flex flex-col justify-between">
                          <p className="text-[#394649] text-[15px] leading-relaxed italic mb-6">"{t.quote}"</p>
                          <p className="font-bold text-black text-[14px]">— {t.name}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <Accordion type="single" collapsible className="w-full bg-white rounded-xl border border-[#E5E5E5] shadow-sm">
                      {salesPage.faqs.map((faq, index) => (
                        <AccordionItem key={index} value={`item-${index}`} className="last:border-b-0">
                          <AccordionTrigger className="px-6 text-[16px] font-bold text-black hover:text-primary hover:no-underline">
                            {faq.question}
                          </AccordionTrigger>
                          <AccordionContent className="px-6 pb-6 text-[#394649] text-[15px] leading-relaxed">
                            {faq.answer}
                          </AccordionContent>
                        </AccordionItem>
                      ))}
              ...
              <div className="lg:col-span-5">
                <div className="sticky top-32 space-y-6">
                  ...
                  <div className="bg-white rounded-2xl border border-[#E5E5E5] shadow-[0_20px_40px_rgba(0,0,0,0.04)] overflow-hidden">
                    <div className="p-8 pb-6 border-b border-[#E5E5E5]">
                      <div className="flex items-center justify-between mb-6">
                        <span className="text-[40px] font-bold text-black leading-none">Free</span>
                        <span className="bg-[#E3F9EF] text-[#10A364] px-3 py-1 rounded-full text-[13px] font-bold">
                          Digital Access
                        </span>
                      </div>
                    ...
                    <div className="p-8 bg-[#FAFAFA] space-y-6">
                      ...
                      {(salesPage?.includedItems && salesPage.includedItems.length > 0) ? (
                        <div className="space-y-4">
                          <h4 className="font-bold text-black flex items-center gap-2 text-[15px]">
                  ...
                  {(salesPage?.supportEmail || salesPage?.terms) && (
                    <div className="pt-4 space-y-3 px-2">
                      {salesPage.supportEmail && (
                        <p className="text-[13px] text-[#4D4D4D] flex items-center gap-2">
                          <Mail className="w-4 h-4 text-[#9794AA]" /> 
                          Questions? <a href={`mailto:${salesPage.supportEmail}`} className="text-primary hover:underline font-medium">{salesPage.supportEmail}</a>
                        </p>
                      )}
                      {salesPage.terms && (
