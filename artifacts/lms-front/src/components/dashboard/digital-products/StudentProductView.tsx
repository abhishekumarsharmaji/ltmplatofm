        ...
        {product.files && product.files.length > 0 ? (
          <div className="grid gap-4">
            {product.files.map(file => (
              <div key={file.id} className="bg-white border border-[#E5E5E5] rounded-xl p-4 flex items-center justify-between shadow-sm hover:shadow-md transition-shadow group">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-[15px] text-black">{file.filename}</h4>
                    <p className="text-[13px] text-[#4D4D4D]">{formatBytes(file.sizeBytes || 0)}</p>
                  </div>
                </div>
                <Button 
                  onClick={() => handleDownload(file.id)}
                  className="h-10 px-5 bg-primary hover:bg-[#10A364] text-white font-medium rounded-md text-[14px] shadow-[0_4px_14px_rgba(21,207,116,0.25)]"
                >
